...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkInexistingProducts.length) {
      const nonExistintProductIds = checkInexistingProducts.map(
        product => product.id,
      );
      throw new AppError(`Could not find products:${nonExistintProductIds}`);
    }
    ...
    if (findProductsWithNoQuantityAvailable.length) {
      throw new AppError(
        `The quantity ${findProductsWithNoQuantityAvailable[0].quantity} is not availavle for ${findProductsWithNoQuantityAvailable[0].id}`,
      );
    }
